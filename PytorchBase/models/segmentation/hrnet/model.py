...
def get_hrnet(model:str="hrnet18"):
    if model == "hrnet18":
        return HighResolutionNet(MODEL_CONFIGS['hrnet18'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)
    elif model == "hrnet32":
        return HighResolutionNet(MODEL_CONFIGS['hrnet32'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)
    elif model == "hrnet48":
        return HighResolutionNet(MODEL_CONFIGS['hrnet48'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)
    elif model == "hrnet64":
        return HighResolutionNet(MODEL_CONFIGS['hrnet64'], 
                                         bn_type='inplace_abn',
                                         bn_momentum=0.1)


class HRNetBackbone(object):
    def __init__(self, model:str="hrnet18"):
        self.model = model
...
class HRNet(nn.Module):
    __in_channels = {
        "hrnet18":270, # 18 + 36 + 72 + 144
        "hrnet32":480, # 32 + 64 + 128 + 256
        "hrnet48":720, # 48 + 96 + 192 + 384
        "hrnet64":860  # 64 + 128 + 256 + 512
    }

    def __init__(self, backbone:str="hrnet18", num_classes:int=18):
        super(HRNet, self).__init__()
        
        if backbone not in self.__in_channels.keys():
            raise NotImplementedError
        
        self.num_classes = num_classes
        self.backbone = get_hrnet(backbone)

        # extra added layers
        in_channels = self.__in_channels[backbone]
...
class HRNet_OCR_B(nn.Module):
    ...
    def __init__(self, backbone:str="hrnet18", num_classes:int=18):
        super(HRNet_OCR_B, self).__init__()
